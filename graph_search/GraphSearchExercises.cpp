...
int GridGraph::countEdges() const {
  ...
  // TODO: Your code here!
  // =======================================================================
  std::unordered_set<IntPairPair> seen_edges;

  for (const auto& v : adjacencyMap){
      for(const auto& w : v.second) {
          IntPairPair p1 = IntPairPair(v.first,w);
          IntPairPair p2 = IntPairPair(w,v.first);

          if (!seen_edges.count(p1) && !seen_edges.count(p2)) {
              numEdges++;
              seen_edges.insert(p1);
              seen_edges.insert(p2);
          }
      }
  }
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  // TODO: Your code here!
  // =======================================================================
  for(const IntPair& p : originalNeighbors){
      adjacencyMap.at(p).erase(p1);
  }
