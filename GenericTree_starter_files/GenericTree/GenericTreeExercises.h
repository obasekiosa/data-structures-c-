...
static void treeFactory(GenericTree<int>& tree) {
  ...
  tree.createRoot(4);
  GenericTree<int>::TreeNode *t4 = tree.getRootPtr();
  t4->addChild(8);
  t4->addChild(15);
  
  std::vector<GenericTree<int>::TreeNode*> &t4child = t4->childrenPtrs;
  t4child[0]->addChild(16);
  t4child[0]->addChild(23);

  std::vector<GenericTree<int>::TreeNode*> &t16child = t4child[0]->childrenPtrs;
  t16child[0]->addChild(42); 
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while(!q.empty()) {
      ...
      for (TreeNode *& node : curr->childrenPtrs) {
          q.push(node);
      }
  }
